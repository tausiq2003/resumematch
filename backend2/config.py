...
class Config:
    ...
    JSEARCH_API_URL = os.getenv(
        "JSEARCH_API_URL", "https://jsearch.p.rapidapi.com/search")  # Default URL
    JSEARCH_API_KEY = os.getenv("JSEARCH_API_KEY", "")
    JSEARCH_API_HOST = os.getenv(
        "JSEARCH_API_HOST", "jsearch.p.rapidapi.com")  # Default host
    JSEARCH_API_RATE_LIMIT_CALLS = int(
        os.getenv("JSEARCH_API_RATE_LIMIT_CALLS", 5))  # Default to 5 calls
    JSEARCH_API_RATE_LIMIT_PERIOD = float(
        os.getenv("JSEARCH_API_RATE_LIMIT_PERIOD", 1))  # Default to 1 second

    REDIS_HOST = os.getenv("REDIS_HOST", "")
    REDIS_PORT = os.getenv("REDIS_PORT", 6379)

    SERP_API_URL = os.getenv(
        "SERP_API_URL", "https://serpapi.com/search.json")  # Default URL
    ...
    if not GEMINI_API_KEY:
        raise RuntimeError("Required environment variable 'GEMINI_API_KEY' is not set.")

    if FETCHER_NAME == "jsearch" and not JSEARCH_API_KEY:
        raise RuntimeError(
            "Required environment variable 'JSEARCH_API_KEY' is not set. This is required when working with the {FETCHER_NAME} fetcher.")
    if FETCHER_NAME == "jsearch" and not REDIS_HOST:
        logger.warning(
            "Redis host is not provided in .env --> defaulting to localhost. Make sure to have a local Redis server functional.")
